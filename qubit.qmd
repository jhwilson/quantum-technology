...
- You cannot directly access of the amplitudes of $\ket{0}$ and $\ket{1}$ through a single measurement.
- Superposition states collapse upon measurement.
- No-cloning theorem @WoottersZurek1982 means you cannot perfectly copy an unknown quantum state.
...
For the qubit, we associate two states with two different basis vectors: $\ket{0}$ and $\ket{1}$. This will be called the _computational basis_. The magic[^5] of quantum mechanics is that a state need not be just one or the other, but could be _any_ linear superposition of these
$$
...
A particularly important set of operators are the Pauli operators. We've already seen the Pauli $Z$ operator. The other two are[^6]
$$
...
## The Bloch sphere {#sec-bloch-sphere}
...
In this case, if we set $\ket{\psi} = x \ket{0} + y e^{i\phi} \ket 1$, then we have $\braket{\psi|\psi} = x^2 + y^2 = 1$ for normalization. This is the equation for a circle, and writing out $x = \cos(\theta/2)$ and $y = \sin(\theta/2)$[^7], we are lead to an angular representation of our state.
$$
...
\end{quantikz}
``` -->
::: {#fig-single-qubit .centered layout-ncol=3}

![Identity on $\ket{0}$](images/tikz/qubit-identity-0.svg){#fig-identity-0}

![Identity on $\ket{1}$](images/tikz/qubit-identity-1.svg){#fig-identity-1}

![Identity on $\ket{\psi}$](images/tikz/qubit-identity.svg){#fig-qubit-identity}
...
::: {.callout-important}
The Pauli operators are both unitary and Hermitian, so they perform as quantum gates (which require unitary as @sec-postulate-unitary stipulates) and as observables (as @sec-postulate-observable stipulates).
:::
...
\end{quantikz}
``` -->
::: {#fig-qubit-XZ-gates .centered layout-ncol=2}

![Application of the $X$ gate.](images/tikz/qubit-X-gate.svg){#fig-qubit-X-gate width="50%"}

![Application of the $Z$ gate.](images/tikz/qubit-Z-gate.svg){#fig-qubit-Z-gate width="50%"}
...
\end{quantikz}
``` -->
::: {#fig-qubit-HS-gates .centered layout-ncol=2}

![Application of the Hadamard gate.](images/tikz/qubit-H-gate.svg){#fig-qubit-H-gate}

![Application of the phase gate](images/tikz/qubit-S-gate.svg){#fig-qubit-phase-gate}
...
\end{quantikz}
``` -->
::: {#fig-qubit-measure .centered layout-ncol=2}

![Will measure 1 with 100% probability](images/tikz/qubit-X-gate-measure.svg){#fig-qubit-X-measure}

![Will measure 1 with 50% probability](images/tikz/qubit-H-gate-measure.svg){#fig-qubit-H-measure}
...
<!-- ```{.tikz}
%%| filename: qubit-HZH-gate-measure
%%| caption: Implementing an $X$ gate with $Z$ and $H$.
%%| header-includes: \usetikzlibrary{quantikz2}
%%| additionalPackages: \usepackage{braket}
\begin{quantikz}
\lstick{$\ket{0}$} & \gate{H} & \gate{Z} & \gate{H} & 
\end{quantikz}
``` -->
![Implementing an $X$ gate with $Z$ and $H$.](images/tikz/qubit-HZH-gate-measure.svg){#fig-X-is-H-Z}


1. $H\ket{0} = \ket{+}$ (move to +x axis)
2. $Z\ket{+} = \frac{1}{\sqrt{2}}(\ket{0} - \ket{1})$ (rotate around z by 90°)
3. $H(\frac{1}{\sqrt{2}}(\ket{0} - \ket{1})) = \ket{1}$ (move to -z axis)
...
The real power of density matrices comes from describing _mixed states_, which are statistical mixtures of pure states:
$$
...
2. Positive semidefinite: $\bra{\phi}\rho\ket{\phi} \geq 0$ for all $\ket{\phi}$
3. Unit trace: $\tr(\rho) = 1$
4. For pure states: $\tr(\rho^2) = 1$
5. For mixed states: $\tr(\rho^2) < 1$
:::

#### Quantum Operations with Density Matrices {.unnumbered}
...
#### Density Matrices and the Bloch Sphere {.unnumbered}

The density matrix formalism provides a beautiful geometric picture when combined with the Bloch sphere representation in @sec-bloch-sphere. While pure states live on the surface of the Bloch sphere, mixed states live _inside_ it. Any density matrix for a single qubit can be written as:
$$
...
A full discussion of different quantum channels can be found in @Wilde2017. Here, we discuss some of the ones relevant for loss of quantum information.

The language of density matrices allows us to describe some of the errors that can occur in a precise manner

#### Random bit-flips {.unnumbered}
...
#### Dephasing {.unnumbered}

As we've already mentioned the phase between $\ket{0}$ and $\ket{1}$ is useful information that we will take advantage of.
However, some physical processes can scramble this phase in a way that could be inherently difficult to parse, in that case we can describe these processes with "dephasing"
$$
...
#### Photonic Qubits {.unnumbered}

- Qubits are properites of light (e.g., polarization)
- Can operate at room temperature
- Used by: PsiQuantum, Xanadu

#### Semiconductor Quantum Dots {.unnumbered}

- Qubits can be encoded using either:
  * _charge_ states (electron occupying different quantum dots)
  * _spin_ states (up/down spin states of an electron)
...
#### NV Centers in Diamond {.unnumbered}
...
[^5]: Magic is a technical term in quantum computing, though we're using it in a colloquial sense here, see @BravyiKitaev2005.
[^6]: In some literature, these are matrices are denoted by $\sigma_{x}$, $\sigma_y$, and $\sigma_z$ and related to spin operators via $S_i =\frac12 \sigma_i$. This insight can help bridge the idea of these operators and the Bloch sphere.
[^7]: The divide-by-two for the angles will become clear as we go through this section.
